...
class AuthNotifier extends StateNotifier<Auth> {
  static const String casualAuthTokenStorageKey = 'casualAuthTokenStorageKey';
  ...
  Future<void> storeToken(String token) async {
    await storage.write(key: casualAuthTokenStorageKey, value: token);
    state = Auth.authenticated(token: token);
  }

  Future<void> clearToken() async {
    await storage.delete(key: casualAuthTokenStorageKey);
    state = Auth.unauthenticated();
  }

  Future<void> _loadInitialValue() async {
    final tokenFromStorage = await storage.read(key: casualAuthTokenStorageKey);
